...
class ValheimServer extends Construct {
  constructor(scope: Construct, id: string, props: ValheimServerProps) {
    ...
    const hasRequestedWorldInResourcesFolder = props.worldAssetLocations?.includes(
      `resources/worlds/${BOOTSTRAP_WITH_WORLD_NAME}`
    )
...
new ValheimServer(app, 'ValheimServer', {
  worldAssetLocations,
  addAppGatewayStartStopStatus: !APPGW_START_STOP_PASSWORD || APPGW_START_STOP_PASSWORD.length === 0 ? false : true,
