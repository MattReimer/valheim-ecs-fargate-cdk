...
export class ValheimServerAwsCdkStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: ValheimServerAwsCdkStackProps) {
    ...
    const valheimTaskDefinition = new TaskDefinition(this, 'valheimTaskDefinition', {
    ...
    if (props && props.worldResourcesBucket) {
      ...
      Annotations.of(this).addInfo(
        'World bootstrapping is configured, if the EFS file system already has a /config/worlds_local/ folder, then we will NOT bootstrap. This is to prevent overrwriting with the original bootstrap if the container restarts.'
      )
    }
    ...
    const container = valheimTaskDefinition.addContainer('valheimContainer', {
      ...
      environment,
      secrets: {
        SERVER_PASS: Secret.fromSecretsManager(valheimServerPass, 'VALHEIM_SERVER_PASS'),
      },
    })
