...
export default class WorldBootstrapResourcesStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: WorldBootstrapResourcesStackProps) {
    ...
    if (worldAssetLocations.length > 1) {
      Annotations.of(bucketDeployment).addWarning(
        "Detected there are more than one world being uploaded. Be careful! Current implementation relies on the root of the bucket containing all of '~/.config/unity3d/IronGate/Valheim/'"
      )
    }
  }
}
