...
async function getIPFunction() {
  ...
  try {
    ...
    const describeNetworkInterfacesResult = await ec2Client.send(
      new DescribeNetworkInterfacesCommand(describeNetworkInterfacesInput)
    )

    if (
      !describeNetworkInterfacesResult.NetworkInterfaces ||
      describeNetworkInterfacesResult.NetworkInterfaces.length <= 0
    )
      return
    const publicIp = describeNetworkInterfacesResult.NetworkInterfaces.find((x) => x.Association != undefined)
...
const getNetworkInterfaceId = async (cluster: string, tasks: string[]): Promise<string> => {
  ...
  try {
    ...
    if (!networkInterfaceId)
      throw new Error(`Could not find a network interface on given attachment: ${JSON.stringify(firstAttachment)}`)
    ...
    throw new Error(
      `Failed to get the first network interface ID for the following tasks ${tasks.join(
        ','
      )} in ${cluster}. Cause: ${JSON.stringify(err)}`
    )
  }
}
